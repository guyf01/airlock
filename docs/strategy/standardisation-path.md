...
Writing a spec proposal without a working system invites debate about whether it is necessary or correct. A proxy with adoption makes the need undeniable and the design concrete.
...
Once the pattern is established and adopted across multiple popular MCP servers, submit to the MCP spec. The language writes itself from what the proxy already implements:
